...
typedef struct _boot_extend_head_t {
	uint8_t		version[8];		/*version:1.0*/
	uint8_t		magic[8];		/* ="DRAM.ext" */
	uint8_t            select_mode;		/*0:不进行自动识别 1:gpio识别模式 2:gpadc识别模式 3:1个IO+gpadc识别模式*/
	uint8_t            gpadc_channel;		/*select gpadc 通道*/
	uint8_t            reserve[2];
